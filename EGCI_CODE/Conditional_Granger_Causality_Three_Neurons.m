function [Var_xy,Var_yx,hh]= Conditional_Granger_Causality_Three_Neurons(pointset,detal,Length_N,m1,m2,m3,Taux,Tauy,Option,count_r,neighbors)
% Length_N 为原始数据的长度，例如：原来采用的y的长度为10，则Length_N即为10；
global Resultsmat;
...
%% ---------------------------
Temp_prb=zeros(length(count_r),2);
for i=1:length(count_r)
    h_0=count_r(i);
        %disp(['The Progress: ',num2str(i)])
if h_0>10*lm
     
    Next_Y_ix=(1+Length_N).*ones(1,h_0)-neighbors{i,1}+Taux.*ones(1,h_0);  
    Next_Y_iy=(1+Length_N).*ones(1,h_0)-neighbors{i,1}+Tauy.*ones(1,h_0);  
    
    index=find(Next_Y_ix<Length_N+1); 
    h_y_ii=length(index);
    Next_Y_ii=(1+Length_N).*ones(1,h_y_ii)-Next_Y_ix(index);% X is the dependent variable
    
    index_y=find(Next_Y_iy<Length_N+1); 
    h_y_ii_y=length(index_y);
    Next_Y_ii_y=(1+Length_N).*ones(1,h_y_ii_y)-Next_Y_iy(index_y);% Y is the dependent variable
