% 
% EGCI for f1=0.28 f2=0.282 S12=Change S21=0.1 Amp1=Amp2=10
clc
clear
% close all
tic;
...
X=[V1(1,1:stv_L:length_N);V2(1,1:stv_L:length_N)];
     ...
     if data_chose==280
V1=load('HH_280/HH_Solution_w1_0.28_0.txt');
V2=load('HH_280/HH_Solution_w1_0.28_1.txt'); 
...
max_k=length(x);
if  SNR_Determined==1
 sigma_white=sqrt(var(y))/10^(3.597)
...
Resultsmat=[pwd,'/',str_name];
 ...
 elseif data_chose==3766
     if Noise_Add==1
         str_name=(['HH_3766/F_Test/Noise_Results_All/mx_',num2str(mx),' my_',num2str(my),'Sigma_',num2str(sigma_white),' Results mat']);    
     else
 str_name=(['HH_3766/F_Test/mx_',num2str(mx),' my_',num2str(my),' Results mat']);
     end
if ~exist(str_name)
    mkdir(str_name)
end
